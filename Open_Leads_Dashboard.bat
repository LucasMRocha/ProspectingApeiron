...
set "LAUNCHER_DIR_ESC="
for /f "usebackq delims=" %%i in (`powershell -NoProfile -Command "$p=[Uri]::EscapeDataString((Resolve-Path -LiteralPath '%ROOT%').Path); Write-Output $p"`) do set "LAUNCHER_DIR_ESC=%%i"
set "URL=http://localhost:%PORT%/%PAGE%?launcher=prospecting&launcherDir=%LAUNCHER_DIR_ESC%"
...
if exist "%EDGE1%" (
  start "" /D "%ROOT%" "%EDGE1%" --app="%URL%" --new-window
  goto :end
)
if exist "%EDGE2%" (
  start "" /D "%ROOT%" "%EDGE2%" --app="%URL%" --new-window
  goto :end
)
if exist "%CHROME1%" (
  start "" /D "%ROOT%" "%CHROME1%" --app="%URL%" --new-window
  goto :end
)
if exist "%CHROME2%" (
  start "" /D "%ROOT%" "%CHROME2%" --app="%URL%" --new-window
  goto :end
)
